...
created: 2026-08-20
updated: 2026-08-20T02:15:16+07:00
---
...
- next_action: monitor Prometheus target health across normal scrape intervals.
...
- timestamp: 2026-08-20T02:10:18+07:00
  checked: local fix validation: `python3 scripts/validate-obs-manifests.py`,
    `python3 -m py_compile scripts/validate-obs-manifests.py`,
    `bash -n scripts/validate-phase-13.sh`, PyYAML parsing of the NetworkPolicy,
    and a controlled static guard regression that removes Alloy from its selector
  found: all local validations pass; the controlled regression fails the static
    guard when Alloy coverage disappears. `validate-phase-13.sh` now checks the
    Alloy target alongside kube-state-metrics and postgres-exporter.
  implication: the uncommitted fix is structurally valid and protected from the
    confirmed selector regression. No server-side diff, apply, or target recovery
    check was performed in this local-only step.

- timestamp: 2026-08-20T02:15:16+07:00
  checked: server-side `kubectl diff` and apply of the observability manifests;
    three consecutive Prometheus target API checks across new 30-second scrape
    timestamps; `timeout 120s bash scripts/validate-phase-13.sh --quick`; and
    current `scrape_samples_scraped` values
  found: the diff showed only creation of `allow-prometheus-target-ingress`, and
    apply created it. Alloy, kube-state-metrics, and postgres-exporter were
    health=up with empty `lastError` in all three target API checks. The quick
    Phase 13 harness passed Alloy, kube-state-metrics, postgres-exporter,
    cAdvisor, `pg_up`, and RabbitMQ. Current scraped samples were Alloy 267,
    kube-state-metrics 6762, and postgres-exporter 1319.
  implication: the narrowly scoped ingress policy restored all three blocked
    Prometheus scrape paths without changing existing ingress or egress rules.
...
- fix: Added an uncommitted minimal ingress NetworkPolicy in
  `k8s/observability/95-netpol-monitoring.yaml`. It selects only alloy,
  kube-state-metrics, and postgres-exporter pods and admits only the Prometheus
  server on TCP 12345, 8080, and 9187. The static guard now rejects missing or
  widened target ingress; live validation now includes Alloy target health.
- verification: Complete. Server-side diff showed only creation of
  `allow-prometheus-target-ingress`, and apply created it. Three consecutive
  Prometheus target API checks across new 30-second scrape timestamps found
  Alloy, kube-state-metrics, and postgres-exporter health=up with empty
  `lastError`. `timeout 120s bash scripts/validate-phase-13.sh --quick` passed,
  including all three targets, cAdvisor, `pg_up`, and RabbitMQ. Current
  `scrape_samples_scraped` values were Alloy 267, kube-state-metrics 6762, and
  postgres-exporter 1319.
- files_changed: `k8s/observability/95-netpol-monitoring.yaml`,
  `scripts/validate-obs-manifests.py`, `scripts/validate-phase-13.sh`, and
  `.planning/debug/observability-targets-down.md`.
